                ...
                script {
                    parallel(
                        'API Test': {
                            ...
                            bat "docker run --name api_test ${IMAGE_NAME}:${TAG} python infra/api_wrapper.py"
                            bat "docker run --name api_test ${IMAGE_NAME}:${TAG} python test/tankerkoenig_stats_api_test.py"
                        },
                        'Change Language': {
